...
async function filterDoctors() {
  ...
  try {
    ...
    list.innerHTML = medicos.map((m, i) => `
      <div class="doctor-option ${i === 0 ? 'selected' : ''}" onclick="selectDoc(this)" data-id="${m.id}" data-especialidad="${m.especialidad}">
...
async function bookAppointment() {
  ...
  const docEl      = document.querySelector('.doctor-option.selected');
  const medicoId   = docEl?.dataset?.id;
  const especialidad = docEl?.dataset?.especialidad || '';
  ...
  try {
    await API.crearCita({ fecha, hora: selectedTime, medicoId, especialidad, tipo, motivo });
